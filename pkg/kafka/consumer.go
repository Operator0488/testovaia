...
func (c *consumer) collectLagMetrics(ctx context.Context) {
	ticker := time.NewTicker(collectMetricsInterval)
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			...
			// Lag по всем партициям
			metrics.KafkaConsumerLag.WithLabelValues(c.config.Topic).Set(float64(stats.Lag))
		}
	}
}
